...
<root>/
  ...
  moved.tf         OpenTofu moved blocks after a post-apply space remap; omitted when nothing moved
  ...
  <space>/         a space — its own nested module directory
    versions.tf
    liftoff.tf
    space.tf       the spacelift_space resource for a space the kit creates; omitted for a destination-account space the stack is mapped onto
    <kind>_<slug>.tf
...
Its `space.tf` holds the space resource for a space the kit creates, parented through a module input rather than a hard-coded reference, so the parent module owns the link.
A destination-account space the batch is mapped onto still gets a module directory, `liftoff.tf`, and `versions.tf`, but no `space.tf`: `local.space_id` is the live space id as a string.
...
`liftoff.tf` is the wiring: it declares the module inputs (`parent_space_id`, `ancestor_space_ids`, `inherited_contexts`), sets `local.space_id` to this space (`spacelift_space.this.id`, the `"root"` literal at the top, or the live destination-space id when mapped), and calls each child space as a module, passing scope down the tree:
...
Placement is a single `local.space_id` — the space is the directory the file lives in.
Map a stack onto an existing destination space with [`transform space mv`](transform.md).
That works before the first publish, and after: restage with `--restage-migrated`, remap, generate (which writes `moved.tf`), publish, and `finalize staged`.
